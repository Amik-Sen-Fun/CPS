...
%   output channels
    data.uChan = i;
    data.JChan = i+2;
    % ttCreateTask( tasknames{2}, periods(i+1) , codefcn{2}, data); % another way for aperiodic tasks
    ttCreatePeriodicTask( tasknames{i+1}, starttimes(i+1), periods(i+1), codefcn{2}, data);
    ttCreateLog(tasknames{i+1},1,['response' num2str(i+1)],1000); % logs if required
    if sched_scheme == "RM"
...
for i = 1:3
  %% your code to attach above deadline-miss handler with each of the above tasks
end